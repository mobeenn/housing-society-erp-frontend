...
const PIE_COLORS = ["#2563eb", "#16a34a", "#f59e0b", "#8b5cf6", "#ef4444", "#0891b2", "#64748b", "#16a34a", "#f97316"];
...
function LoadingState() {
  return <div className="grid gap-5 sm:grid-cols-2 xl:grid-cols-4">{Array.from({ length: 8 }, (_, index) => <div key={index} className="h-32 animate-pulse rounded-xl bg-white" />)}</div>;
}
...
function StatCard({ stat }) {
  ...
  return <Card><div className="flex items-start justify-between gap-3"><div><p className="text-sm text-neutral-500">{stat.label}</p><p className="mt-2 text-2xl font-bold text-neutral-900">{formatValue(stat.value, stat.format)}</p></div><span className="rounded-xl bg-primary-50 p-2.5 text-primary-700"><Icon className="h-5 w-5" /></span></div></Card>;
}
...
function ListCard({ title, items, type }) {
  ...
  return <Card title={title}><div className="space-y-3">{items.length === 0 ? <p className="py-5 text-center text-sm text-neutral-400">No records available.</p> : items.map((item) => <Link key={item._id || item.complaintNumber || item.memberId} to={href(item)} className="block rounded-lg border border-neutral-100 p-3 hover:border-primary-200 hover:bg-primary-50/30"><div className="flex items-center justify-between gap-3"><p className="truncate text-sm font-medium text-neutral-800">{isPayment ? item.memberName || item.receiptNumber : isComplaint ? item.complaintNumber || "Complaint" : item.memberName || item.name}</p>{item.status && <StatusPill status={item.status} />}</div><p className="mt-1 truncate text-xs text-neutral-400">{isPayment ? `${item.receiptNumber} · ${formatValue(item.amount, "currency")}` : isComplaint ? `${item.category} · ${formatDate(item.createdAt)}` : item.subtitle || item.email || ""}</p></Link>)}</div></Card>;
}
...
export default function DashboardPage() {
  ...
  if (isError) return <Card><div className="py-12 text-center"><AlertCircle className="mx-auto h-10 w-10 text-danger-600" /><p className="mt-3 font-semibold">Dashboard data could not be loaded.</p><button onClick={() => refetch()} className="mt-3 text-sm text-primary-600 underline">Retry</button></div></Card>;
  ...
  return <div className="space-y-6">
    <div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-end"><div><div className="flex items-center gap-3"><h1 className="text-2xl font-bold text-neutral-900" data-tour="dashboards-page">{roleLabel(user)} Dashboard</h1><span className="rounded-full bg-success-50 px-2.5 py-1 text-xs font-medium text-success-700">Live backend data</span></div><p className="mt-1 text-sm text-neutral-500" data-tour="inventory-dashboard">Role-specific operational overview for {roleLabel(user)}.</p></div><p className="text-xs text-neutral-400">Updated {formatDate(data.generatedAt)}</p></div>
    ...
    <div className="grid gap-5 lg:grid-cols-3" data-tour="dashboards-charts">
      <Card title="Trend" className="lg:col-span-2"><div className="h-72"><ResponsiveContainer width="100%" height="100%"><BarChart data={trendData}><CartesianGrid strokeDasharray="3 3" stroke="#e5e7eb" /><XAxis dataKey={trend.x} tickLine={false} axisLine={false} /><YAxis tickLine={false} axisLine={false} tickFormatter={(value) => compactFormatter.format(value)} /><Tooltip contentStyle={{ borderRadius: 10, border: "1px solid #e5e7eb" }} /><Legend /><Bar dataKey={trend.primary} name={trend.primaryLabel} fill="#2563eb" radius={[4, 4, 0, 0]} /><Bar dataKey={trend.secondary} name={trend.secondaryLabel} fill="#8b5cf6" radius={[4, 4, 0, 0]} /></BarChart></ResponsiveContainer></div></Card>
      <Card title="Distribution"><div className="h-72"><ResponsiveContainer width="100%" height="100%"><PieChart><Pie data={distribution} dataKey="count" nameKey="label" cx="50%" cy="50%" innerRadius={50} outerRadius={80} paddingAngle={3}>{distribution.map((item, index) => <Cell key={item.label} fill={PIE_COLORS[index % PIE_COLORS.length]} />)}</Pie><Tooltip /><Legend layout="vertical" align="right" iconType="circle" /></PieChart></ResponsiveContainer></div></Card>
    </div>
    <div className="grid gap-5 lg:grid-cols-2" data-tour="dashboards-lists"><ListCard {...listConfig} />{lowStock.length > 0 || dashboardType === "management" || dashboardType === "operations" ? (canViewInventory && <LowStockWidget items={lowStock} />) : <Card title="Quick Links"><div className="grid gap-2 sm:grid-cols-2">{canViewReports && <Link to="/reports" className="rounded-lg border border-neutral-200 p-3 text-sm font-medium text-primary-700 hover:bg-primary-50">Open Reports</Link>}{canViewNotices && <Link to="/notices" className="rounded-lg border border-neutral-200 p-3 text-sm font-medium text-primary-700 hover:bg-primary-50">View Notices</Link>}</div></Card>}</div>
    {canViewReports && <p className="text-xs text-neutral-400">Use the Reports hub for detailed cross-module analysis and exports.</p>}
  </div>;
}
