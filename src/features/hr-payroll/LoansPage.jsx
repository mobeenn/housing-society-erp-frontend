    ...
    <div className="space-y-5" data-tour="hr-payroll-loans-page">
      <Card title="Employee Loans" actions={canCreate && <Button data-tour="hr-payroll-disburse" onClick={() => setIsModalOpen(true)}><Plus className="h-4 w-4" /> Disburse Loan</Button>}>
        <div className="grid gap-3 md:grid-cols-[1fr_180px]" data-tour="hr-payroll-loan-filters">
          <div className="relative"><Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted" /><Input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search employee..." className="pl-9" /></div>
          <select value={status} onChange={(event) => setStatus(event.target.value)} className="rounded-control border border-border-strong px-3 py-2 text-body">
            ...
            <table className="w-full text-left text-body">
              ...
              <tbody className="divide-y divide-border">
                {visibleLoans.map((loan) => <tr key={loan._id}>
                  <td className="px-3 py-4"><p className="font-medium text-primary">{loan.employeeRef?.name || "Unknown"}</p><p className="text-small text-secondary">{loan.employeeRef?.employeeId || "—"}</p></td>
                  ...
                  <td className="px-3 py-4"><span className={`rounded-full px-2.5 py-1 text-small font-medium ${statusClass(loan.status)}`}>{loan.status}</span></td>
