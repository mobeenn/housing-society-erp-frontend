  ...
  return (
    <div className="space-y-5">
      <Card title="Salary Components" actions={canEdit && <Button size="sm" variant="outline" onClick={addComponent}><Plus className="h-4 w-4" /> Add Component</Button>}>
        <p className="mb-4 text-body text-secondary">Define how each employee's salary structure amount is interpreted.</p>
        <div className="space-y-2">
          {form.salaryComponents.map((component, index) => (
            <div key={`${component.name}-${index}`} className="grid gap-2 md:grid-cols-[1fr_150px_180px_40px]">
              ...
              <label className="text-body font-medium text-primary">Type
                <select value={component.type} disabled={!canEdit} onChange={(event) => updateComponent(index, "type", event.target.value)} className="mt-1 w-full rounded-control border border-border-strong px-3 py-2 text-body disabled:bg-surface-muted">
                  <option value="Earning">Earning</option><option value="Deduction">Deduction</option>
                </select>
              </label>
              <label className="text-body font-medium text-primary">Calculation
                <select value={component.calculationType} disabled={!canEdit} onChange={(event) => updateComponent(index, "calculationType", event.target.value)} className="mt-1 w-full rounded-control border border-border-strong px-3 py-2 text-body disabled:bg-surface-muted">
                  <option value="Fixed">Fixed</option><option value="Percentage">Percentage</option>
                </select>
              </label>
              <button type="button" disabled={!canEdit} onClick={() => removeComponent(index)} className="mt-6 rounded-control p-2 text-danger hover:bg-danger-soft disabled:opacity-40" aria-label="Remove component"><Trash2 className="h-4 w-4" /></button>
            </div>
          ))}
          {form.salaryComponents.length === 0 && <p className="rounded-control bg-canvas p-4 text-body text-secondary">No salary components configured.</p>}
        </div>
      </Card>
      ...
      <Card title="Statutory Contributions">
        ...
        <p className="mt-3 text-small text-secondary">Contributions are calculated from prorated gross earnings and posted as liabilities when the run is approved.</p>
      </Card>
      ...
      <Card title="Progressive Tax Slabs" actions={canEdit && <Button size="sm" variant="outline" onClick={addSlab}><Plus className="h-4 w-4" /> Add Slab</Button>}>
        <p className="mb-4 text-body text-secondary">Enter annual taxable-income slabs. Leave the upper bound blank for the final open-ended slab.</p>
        <div className="space-y-3">
          {form.taxSlabs.map((slab, index) => (
            <div key={`slab-${index}`} className="grid gap-2 md:grid-cols-[1fr_1fr_1fr_40px]">
              ...
              <button type="button" disabled={!canEdit} onClick={() => removeSlab(index)} className="mt-6 rounded-control p-2 text-danger hover:bg-danger-soft disabled:opacity-40" aria-label="Remove tax slab"><Trash2 className="h-4 w-4" /></button>
            </div>
          ))}
          {form.taxSlabs.length === 0 && <p className="rounded-control bg-canvas p-4 text-body text-secondary">No tax slabs configured; tax will be zero.</p>}
        </div>
      </Card>
