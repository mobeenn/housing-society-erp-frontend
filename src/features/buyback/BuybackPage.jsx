  ...
  return (
    <div className="space-y-5" data-tour="buyback-page">
      <Card title="Buyback / Cancel Booking" actions={<Button variant="outline" size="sm" onClick={load}><RefreshCw className="h-4 w-4" /> Refresh</Button>}>
        <div className="mb-4 flex items-start gap-2 rounded-lg bg-red-50 p-3 text-sm text-red-700" data-tour="buyback-warning"><ShieldAlert className="mt-0.5 h-4 w-4 shrink-0" /><span>This action is irreversible. Buyback releases the plot to <strong>Available</strong>; Cancel marks the plot <strong>Cancelled</strong>. The booking is archived, never deleted.</span></div>
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
          <label className="text-sm font-medium text-neutral-700 lg:col-span-2" data-tour="buyback-booking">Booking<select value={form.booking} onChange={(event) => { update("booking", event.target.value); const booking = eligibleBookings.find((item) => item._id === event.target.value); if (booking) update("settlementAmount", String(booking.bookingAmount || booking.price || 0)); }} className="mt-1 w-full rounded-lg border border-neutral-300 px-3 py-2 text-sm"><option value="">Select an active booking</option>{eligibleBookings.map((booking) => <option key={booking._id} value={booking._id}>{booking.bookingNumber || booking._id} · Plot {booking.plotNumber || booking.plot}</option>)}</select></label>
          <label className="text-sm font-medium text-neutral-700" data-tour="buyback-action">Action<select value={form.type} onChange={(event) => update("type", event.target.value)} className="mt-1 w-full rounded-lg border border-neutral-300 px-3 py-2 text-sm"><option value="BuyBack">BuyBack</option><option value="Cancel">Cancel</option></select></label>
          <label className="text-sm font-medium text-neutral-700">Payment type<select value={form.paymentType} onChange={(event) => update("paymentType", event.target.value)} className="mt-1 w-full rounded-lg border border-neutral-300 px-3 py-2 text-sm"><option>Cash</option><option>Adjustment</option><option>Online</option></select></label>
      ...
      {canView && <Card title="Action History"><div className="overflow-x-auto" data-tour="buyback-history"><table className="w-full text-left text-sm"><thead className="border-b border-neutral-200 text-xs uppercase text-neutral-500"><tr><th className="px-3 py-3">Date</th><th className="px-3 py-3">Plot</th><th className="px-3 py-3">Booking</th><th className="px-3 py-3">Type</th><th className="px-3 py-3">Settlement</th><th className="px-3 py-3">Status</th><th className="px-3 py-3">Invoice</th></tr></thead><tbody className="divide-y divide-neutral-100">{records.map((record) => <tr key={record._id}><td className="px-3 py-3">{new Date(record.date).toLocaleString()}</td><td className="px-3 py-3">{record.plotRef?.plotNumber || record.plot}</td><td className="px-3 py-3">{record.booking}</td><td className="px-3 py-3">{record.type}</td><td className="px-3 py-3">{record.settlementAmount}</td><td className="px-3 py-3"><span className={`rounded-full px-2 py-1 text-xs ${lifecycleStatusClass(record.status)}`}>{record.status}</span></td><td className="px-3 py-3">{record.invoiceUrl && <Button size="sm" variant="outline" onClick={() => openInvoiceFile(record.invoiceUrl).catch((error) => toast.error(error.message))}><FileText className="h-4 w-4" /> View</Button>}</td></tr>)}{records.length === 0 && <tr><td colSpan="7" className="px-3 py-8 text-center text-neutral-500">No buyback records.</td></tr>}</tbody></table></div></Card>}
