  ...
  return (
    <div className="space-y-5" data-tour="plot-merge-page">
      <Card title="Plot Merge" actions={<Button variant="outline" size="sm" onClick={load}><RefreshCw className="h-4 w-4" /> Refresh</Button>}>
        <div className="mb-4 flex items-start gap-2 rounded-lg bg-red-50 p-3 text-sm text-red-700" data-tour="plot-merge-warning"><ShieldAlert className="mt-0.5 h-4 w-4 shrink-0" /><span>This action is irreversible. The first selected plot is retained as the resulting unit; all other selected plots will be marked <strong>Merged</strong>.</span></div>
        <div className="grid gap-4 lg:grid-cols-[1fr_280px]">
          <div>
            <p className="mb-2 text-sm font-medium text-neutral-700">Select plots</p>
            <div className="max-h-72 space-y-2 overflow-y-auto rounded-lg border border-neutral-200 p-2" data-tour="plot-merge-plot-selection">
              {loading ? <p className="p-3 text-sm text-neutral-500">Loading plots...</p> : availablePlots.map((plot) => <label key={plot._id} className="flex cursor-pointer items-center gap-3 rounded-lg p-2 hover:bg-neutral-50"><input type="checkbox" checked={selected.includes(plot._id)} onChange={() => togglePlot(plot._id)} className="h-4 w-4" /><span className="flex-1"><span className="block text-sm font-medium text-neutral-900">{plot.plotNumber} {plot.fileNumber ? `(${plot.fileNumber})` : ""}</span><span className="text-xs text-neutral-500">{plot.block?.name || plot.block} · {plot.street?.name || plot.street} · {plot.size}</span></span><span className={`rounded-full px-2 py-0.5 text-xs ${lifecycleStatusClass(plot.status)}`}>{plot.status}</span></label>)}
              {!loading && availablePlots.length === 0 && <p className="p-3 text-sm text-neutral-500">No eligible plots found.</p>}
            </div>
          </div>
          <div className="space-y-3">
            <label className="block text-sm font-medium text-neutral-700" data-tour="plot-merge-result">Resulting plot<select value={resultingPlot} onChange={(event) => setResultingPlot(event.target.value)} className="mt-1 w-full rounded-lg border border-neutral-300 px-3 py-2 text-sm"><option value="">Select resulting plot</option>{selected.map((id) => { const plot = plots.find((item) => item._id === id); return <option key={id} value={id}>{plot?.plotNumber}</option>; })}</select></label>
      ...
      {canView && <Card title="Merge History"><div className="overflow-x-auto" data-tour="plot-merge-history"><table className="w-full text-left text-sm"><thead className="border-b border-neutral-200 text-xs uppercase text-neutral-500"><tr><th className="px-3 py-3">Date</th><th className="px-3 py-3">Resulting plot</th><th className="px-3 py-3">Sources</th><th className="px-3 py-3">Status</th><th className="px-3 py-3">Invoice</th></tr></thead><tbody className="divide-y divide-neutral-100">{merges.map((merge) => <tr key={merge._id}><td className="px-3 py-3">{new Date(merge.date).toLocaleString()}</td><td className="px-3 py-3">{merge.resultingPlotRef?.plotNumber || merge.resultingPlot}</td><td className="px-3 py-3">{merge.plotRefs?.map((plot) => plot.plotNumber).filter(Boolean).join(", ")}</td><td className="px-3 py-3"><span className={`rounded-full px-2 py-1 text-xs ${lifecycleStatusClass(merge.status)}`}>{merge.status}</span></td><td className="px-3 py-3">{merge.invoiceUrl && <Button size="sm" variant="outline" onClick={() => openInvoiceFile(merge.invoiceUrl).catch((error) => toast.error(error.message))}><FileText className="h-4 w-4" /> View</Button>}</td></tr>)}{merges.length === 0 && <tr><td colSpan="5" className="px-3 py-8 text-center text-neutral-500">No merges recorded.</td></tr>}</tbody></table></div></Card>}
