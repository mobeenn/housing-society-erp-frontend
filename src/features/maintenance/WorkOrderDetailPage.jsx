      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
        <section className="space-y-4 rounded-card border border-border bg-surface p-5 shadow-none lg:col-span-2">
          ...
          <dl className="grid grid-cols-2 gap-x-6 gap-y-3 text-body md:grid-cols-3">
            <div>
              <dt className="text-secondary">Asset</dt>
            ...
            <div>
              <dt className="text-secondary">Source complaint</dt>
              <dd className="font-medium">
            ...
            <div>
              <dt className="text-secondary">Expected completion</dt>
              <dd className="font-medium">
            ...
            <div>
              <dt className="text-secondary">Labor cost</dt>
              <dd className="font-medium">{workOrder.laborCost || 0}</dd>
            </div>
            <div>
              <dt className="text-secondary">Material cost</dt>
              ...
              <ul className="divide-y divide-border text-body">
                {workOrder.materials.map((material, index) => (
                  <li key={index} className="flex justify-between py-1.5">
                    <span className="text-primary">{material.item}</span>
          ...
          {/* Progress log */}
          <div className="border-t border-border pt-4" data-tour="maintenance-progress-log">
            ...
            <div className="space-y-3">
              {(workOrder.progressLog || []).map((entry, index) => (
                <div key={index} className="rounded-control bg-canvas p-3">
                  ...
                  <p className="mt-1 text-body text-primary">{entry.note}</p>
                </div>
              ))}
            ...
            {editable && (
              <form onSubmit={submitNote} className="mt-4 flex gap-2">
                ...
                />
                <button data-tour="maintenance-progress-submit" className="rounded-control bg-accent px-4 py-2 text-body font-medium text-on-accent">
                  Add
                </button>
              </form>
            )}
        ...
        <div className="space-y-6">
          <section className="rounded-card border border-border bg-surface p-5 shadow-none">
            ...
            <dl className="space-y-3 text-body">
              ...
              <div>
                <dt className="text-secondary">Contractor</dt>
          ...
          {workOrder.asset && (
            <section className="rounded-card border border-border bg-surface p-5 shadow-none">
              ...
              <button
                onClick={() => navigate(`/assets?id=${workOrder.asset}`)}
                className="w-full rounded-control bg-accent px-4 py-2 text-body font-medium text-on-accent"
              >
